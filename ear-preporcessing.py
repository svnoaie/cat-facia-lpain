...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul 24 22:15:15 2022
...
for e in labelset:
  print(e)
  if  os.path.exists('update/trainear/' + e)==False:
    os.mkdir('update/trainear/'+e)
  if  os.path.exists('update/testear/' + e) ==False:
    os.mkdir('update/testear/'+e)
  if  os.path.exists('update/valear/' + e) ==False:
    os.mkdir('update/valear/'+e)
#
for item in train_list:
  img_path = item[0]
  print("trainear:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/trainear/'+label+'/'+img_name,img)


for item in val_list:
  img_path = item[0]
  print("valear:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/valear/'+label+'/'+img_name,img)

for item in test_list:
  img_path = item[0]
  print("testear:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/testear/'+label+'/'+img_name,img)