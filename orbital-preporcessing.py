...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul 24 22:56:31 2022
...
labels_df = df['orbital_tightening']
label_list = []
...
for e in labelset:
  print(e)
  if  os.path.exists('update/trainorbital/' + e)==False:
    os.mkdir('update/trainorbital/'+e)
  if  os.path.exists('update/testorbital/' + e) ==False:
    os.mkdir('update/testorbital/'+e)
  if  os.path.exists('update/valorbital/' + e) ==False:
    os.mkdir('update/valorbital/'+e)
#
for item in train_list:
  img_path = item[0]
  print("trainorbital:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/trainorbital/'+label+'/'+img_name,img)


for item in val_list:
  img_path = item[0]
  print("valorbital:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/valorbital/'+label+'/'+img_name,img)

for item in test_list:
  img_path = item[0]
  print("testorbital:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/testorbital/'+label+'/'+img_name,img)