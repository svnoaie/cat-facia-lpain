...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul 24 22:51:52 2022
...
training_set = train_datagen.flow_from_directory('update/trainhead',
                                                 target_size = (256, 256),
                                                 batch_size = 32,
                                                 class_mode = 'categorical')#categorical

val_datagen = ImageDataGenerator(rescale = 1.255)
val_set = val_datagen.flow_from_directory('update/valhead',
                                            target_size = (256, 256),
                                            batch_size = 32,
...
test_set = test_datagen.flow_from_directory('update/testhead',
                                            target_size = (256, 256),
                                            batch_size = 32,
                                            class_mode = 'categorical')
#%%
...
cnnhead = tf.keras.models.Sequential()

# Step 1 - Convolution
cnnhead.add(tf.keras.layers.Conv2D(filters=16, kernel_size=3, activation='relu', input_shape=[256, 256, 3]))
cnnhead.add(tf.keras.layers.Conv2D(filters=16, kernel_size=3, activation='relu'))
cnnhead.add(tf.keras.layers.Conv2D(filters=16, kernel_size=3, activation='relu'))
# Step 2 - Pooling
cnnhead.add(tf.keras.layers.MaxPool2D(pool_size=2, strides=2))

# Adding a second convolutional layer
cnnhead.add(tf.keras.layers.Conv2D(filters=32, kernel_size=3, activation='relu'))
cnnhead.add(tf.keras.layers.Conv2D(filters=32, kernel_size=3, activation='relu'))
cnnhead.add(tf.keras.layers.Conv2D(filters=32, kernel_size=3, activation='relu'))
cnnhead.add(tf.keras.layers.MaxPool2D(pool_size=2, strides=2))
cnnhead.add(tf.keras.layers.Conv2D(filters=128, kernel_size=3, activation='relu'))
cnnhead.add(tf.keras.layers.Conv2D(filters=128, kernel_size=3, activation='relu'))
cnnhead.add(tf.keras.layers.Conv2D(filters=128, kernel_size=3, activation='relu'))
cnnhead.add(tf.keras.layers.MaxPool2D(pool_size=2, strides=2))
cnnhead.add(tf.keras.layers.Conv2D(filters=256, kernel_size=3, activation='relu'))
cnnhead.add(tf.keras.layers.MaxPool2D(pool_size=2, strides=2))
# Step 3 - Flattening
cnnhead.add(tf.keras.layers.Flatten())

# Step 4 - Full Connection
cnnhead.add(tf.keras.layers.Dense(units=128, activation='relu'))

# Step 5 - Output Layer
cnnhead.add(tf.keras.layers.Dense(units=5,activation = 'softmax'))
cnnhead.summary()
...
#             metrics = ['accuracy'])
cnnhead.compile(optimizer = 'sgd', loss = 'categorical_crossentropy', metrics = ['accuracy'])

# tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)
# Training the CNN on the Training set and evaluating it on the Test set
history =cnnhead.fit(x = training_set, validation_data = val_set, epochs =1)
...
#%%
Cnnhead=tf.keras.models.load_model('headcustom.h5')
import numpy as np
import os
...
print(len(test_set))
Y_pred = Cnnhead.predict_generator(test_set)
print(Y_pred.shape)
...
import matplotlib.pyplot as plt
import numpy as np
Y_pred = Cnnhead.predict_generator(test_set)#, 1599 // 32+1)
y_pred = np.argmax(Y_pred,axis=1)
...
plt.title("confusion matrix test-head")
plt.show()

#%%
test=os.listdir("update/testhead/action_unit_absent_(0)")
#%%
...
test_image = keras.utils.load_img('update/testhead/action_unit_absent_(0)/'+ test[i], target_size = (256, 256, 3))
#keras.utils
test_image = keras.utils.img_to_array(test_image)
#test_image = image.load_img('update/testhair/short_hair/'+ test[i], target_size = (256, 256, 3))
#test_image = image.img_to_array(test_image)
test_image = np.expand_dims(test_image, axis = 0)
result = Cnnhead.predict(test_image)
image=cv2.imread('update/testhead/action_unit_absent_(0)/'+ test[i])
