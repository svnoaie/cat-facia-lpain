...
for e in labelset:
  print(e)
  if  os.path.exists('update/trainhair/' + e)==False:
    os.mkdir('update/trainhair/'+e)
  if  os.path.exists('update/testhair/' + e) ==False:
    os.mkdir('update/testhair/'+e)
  if  os.path.exists('update/valhair/' + e) ==False:
    os.mkdir('update/valhair/'+e)
#
for item in train_list:
  img_path = item[0]
  print("trainhair:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/trainhair/'+label+'/'+img_name,img)


for item in val_list:
  img_path = item[0]
  print("valhair:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/valhair/'+label+'/'+img_name,img)

for item in test_list:
  img_path = item[0]
  print("testhair:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/testhair/'+label+'/'+img_name,img)