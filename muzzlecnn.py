...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul 24 23:08:32 2022
...
training_set = train_datagen.flow_from_directory('update/trainmuzzle',
                                                 target_size = (256, 256),
                                                 batch_size = 32,
                                                 class_mode = 'categorical')#categorical

val_datagen = ImageDataGenerator(rescale = 1.255)
val_set = val_datagen.flow_from_directory('update/valmuzzle',
                                            target_size = (256, 256),
                                            batch_size = 32,
...
test_set = test_datagen.flow_from_directory('update/testmuzzle',
                                            target_size = (256, 256),
                                            batch_size = 32,
                                            class_mode = 'categorical')
#%%
...
cnnmuzzle = tf.keras.models.Sequential()

# Step 1 - Convolution
cnnmuzzle.add(tf.keras.layers.Conv2D(filters=16, kernel_size=3, activation='relu', input_shape=[256, 256, 3]))
cnnmuzzle.add(tf.keras.layers.Conv2D(filters=16, kernel_size=3, activation='relu'))
cnnmuzzle.add(tf.keras.layers.Conv2D(filters=16, kernel_size=3, activation='relu'))
# Step 2 - Pooling
cnnmuzzle.add(tf.keras.layers.MaxPool2D(pool_size=2, strides=2))

# Adding a second convolutional layer
cnnmuzzle.add(tf.keras.layers.Conv2D(filters=32, kernel_size=3, activation='relu'))
cnnmuzzle.add(tf.keras.layers.Conv2D(filters=32, kernel_size=3, activation='relu'))
cnnmuzzle.add(tf.keras.layers.Conv2D(filters=32, kernel_size=3, activation='relu'))
cnnmuzzle.add(tf.keras.layers.MaxPool2D(pool_size=2, strides=2))
cnnmuzzle.add(tf.keras.layers.Conv2D(filters=128, kernel_size=3, activation='relu'))
cnnmuzzle.add(tf.keras.layers.Conv2D(filters=128, kernel_size=3, activation='relu'))
cnnmuzzle.add(tf.keras.layers.Conv2D(filters=128, kernel_size=3, activation='relu'))
cnnmuzzle.add(tf.keras.layers.MaxPool2D(pool_size=2, strides=2))
cnnmuzzle.add(tf.keras.layers.Conv2D(filters=256, kernel_size=3, activation='relu'))
cnnmuzzle.add(tf.keras.layers.MaxPool2D(pool_size=2, strides=2))
# Step 3 - Flattening
cnnmuzzle.add(tf.keras.layers.Flatten())

# Step 4 - Full Connection
cnnmuzzle.add(tf.keras.layers.Dense(units=128, activation='relu'))

# Step 5 - Output Layer
cnnmuzzle.add(tf.keras.layers.Dense(units=5,activation = 'softmax'))
cnnmuzzle.summary()
...
#             metrics = ['accuracy'])
cnnmuzzle.compile(optimizer = 'sgd', loss = 'categorical_crossentropy', metrics = ['accuracy'])

# tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)
# Training the CNN on the Training set and evaluating it on the Test set
history =cnnmuzzle.fit(x = training_set, validation_data = val_set, epochs =3)
...
#%%4
import os
path = 'muzzlecustom.h5'

cnnmuzzle.save(path)

#%%
Cnnmuzzle=tf.keras.models.load_model('muzzlecustom.h5')
import numpy as np
import os
...
print(len(test_set))
Y_pred = Cnnmuzzle.predict_generator(test_set)
print(Y_pred.shape)
...
import matplotlib.pyplot as plt
import numpy as np
Y_pred = Cnnmuzzle.predict_generator(test_set)#, 1599 // 32+1)
y_pred = np.argmax(Y_pred,axis=1)
...
plt.title("confusion matrix test-muzzle")
plt.show()

#%%
test=os.listdir("update/testmuzzle/action_unit_absent_(0)")
#%%
...
test_image = keras.utils.load_img('update/testmuzzle/action_unit_absent_(0)/'+ test[i], target_size = (256, 256, 3))
#keras.utils
test_image = keras.utils.img_to_array(test_image)
#test_image = image.load_img('update/testhair/short_hair/'+ test[i], target_size = (256, 256, 3))
#test_image = image.img_to_array(test_image)
test_image = np.expand_dims(test_image, axis = 0)
result = Cnnmuzzle.predict(test_image)
image=cv2.imread('update/testmuzzle/action_unit_absent_(0)/'+ test[i])
