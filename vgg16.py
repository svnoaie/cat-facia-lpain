...
batch_size = 24
import sys
sys.stdout = open('./results/vgg16_{}.txt'.format(epochs),'w')
...
model.summary()
#%%
history =model.fit(x=training_set, validation_data= val_set,epochs=epochs)
...
data ={'train_acc':acc,
       ...
       'train_loss':loss,
       'val_loss':val_loss}
pd.DataFrame(data).to_csv('./results/vgg16_h_{}.csv'.format(epochs))
...
print(yest, y_pred)
pd.DataFrame({'label':yest,'pred':y_pred}).to_csv('./results/vgg16_labels_{}.csv'.format(epochs))
