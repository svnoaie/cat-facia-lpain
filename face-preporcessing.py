...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 25 00:04:30 2022
...
for e in labelset:
  print(e)
  if  os.path.exists('update/trainface/' + e)==False:
    os.mkdir('update/trainface/'+e)
  if  os.path.exists('update/testface/' + e) ==False:
    os.mkdir('update/testface/'+e)
  if  os.path.exists('update/valface/' + e) ==False:
    os.mkdir('update/valface/'+e)
#
for item in train_list:
  img_path = item[0]
  print("trainface:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/trainface/'+label+'/'+img_name,img)


for item in val_list:
  img_path = item[0]
  print("valface:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/valface/'+label+'/'+img_name,img)

for item in test_list:
  img_path = item[0]
  print("testface:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/testface/'+label+'/'+img_name,img)