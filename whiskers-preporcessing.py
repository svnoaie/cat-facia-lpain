...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul 24 23:19:14 2022
...
labels_df = df['whiskers_position']
label_list = []
...
for e in labelset:
  print(e)
  if  os.path.exists('update/trainwhiskers/' + e)==False:
    os.mkdir('update/trainwhiskers/'+e)
  if  os.path.exists('update/testwhiskers/' + e) ==False:
    os.mkdir('update/testwhiskers/'+e)
  if  os.path.exists('update/valwhiskers/' + e) ==False:
    os.mkdir('update/valwhiskers/'+e)
#
for item in train_list:
  img_path = item[0]
  print("trainwhiskers:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/trainwhiskers/'+label+'/'+img_name,img)


for item in val_list:
  img_path = item[0]
  print("valwhiskers:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/valwhiskers/'+label+'/'+img_name,img)

for item in test_list:
  img_path = item[0]
  print("testwhiskers:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/testwhiskers/'+label+'/'+img_name,img)