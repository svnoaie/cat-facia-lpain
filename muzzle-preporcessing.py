...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul 24 23:06:48 2022
...
for e in labelset:
  print(e)
  if  os.path.exists('update/trainmuzzle/' + e)==False:
    os.mkdir('update/trainmuzzle/'+e)
  if  os.path.exists('update/testmuzzle/' + e) ==False:
    os.mkdir('update/testmuzzle/'+e)
  if  os.path.exists('update/valmuzzle/' + e) ==False:
    os.mkdir('update/valmuzzle/'+e)
#
for item in train_list:
  img_path = item[0]
  print("trainmuzzle:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/trainmuzzle/'+label+'/'+img_name,img)


for item in val_list:
  img_path = item[0]
  print("valmuzzle:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/valmuzzle/'+label+'/'+img_name,img)

for item in test_list:
  img_path = item[0]
  print("testmuzzle:",img_path)
  label = item[1]
  img = cv2.imread(img_path)

  img_name = img_path.split('/')[-1]

  cv2.imwrite('update/testmuzzle/'+label+'/'+img_name,img)